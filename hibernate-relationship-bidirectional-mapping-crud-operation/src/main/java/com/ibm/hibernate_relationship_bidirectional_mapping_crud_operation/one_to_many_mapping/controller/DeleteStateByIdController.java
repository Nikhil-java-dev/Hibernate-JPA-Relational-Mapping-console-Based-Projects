...
public class DeleteStateByIdController {
	...
	private static final Logger log = LoggerFactory.getLogger(DeleteStateByIdController.class);
	
	public static void main(String[] args) {
		CountryStateDao dao = new CountryStateDao();
		int stateIdToDelete = 4; // Specify the state ID to delete
		
		try {
			boolean isDeleted = dao.deleteStateById(stateIdToDelete);
			if(isDeleted) {
				log.info("State with ID {} deleted successfully.", stateIdToDelete);
			} else {
			    log.warn("No State found with ID {}", stateIdToDelete);
			}
